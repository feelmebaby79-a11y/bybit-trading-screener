...
function displacement(candles,i,direction){
  ...
  return directional&&r>=Math.max(ar*1.35,1e-12)&&body(c)>=Math.max(ab*1.5,r*0.55)&&closeLocation>=0.70;
}

function mostRecentPoiArrival(cs,lo,hi,lookback=80){
  const start=Math.max(1,cs.length-lookback);let arrival=null;
  for(let i=start;i<cs.length;i++){const now=overlap(cs[i],lo,hi),prev=overlap(cs[i-1],lo,hi);if(now&&!prev)arrival=i;}
  return arrival;
}
...
function nearestExpansionLiquidity(pv,entry,dir,fromI){
  ...
  pool.sort((a,b)=>Math.abs(a.price-entry)-Math.abs(b.price-entry));
  return pool[0];
}
...
function findSetup(candles,item,tf){
  ...
  for(const z of zones){
    ...
    for(let i=z.created_i+1;i<cs.length;i++)if(overlap(cs[i],z.low,z.high)){firstTouchI=i;break;}
    if(firstTouchI===null||firstTouchI!==latestI)continue;
    const c=cs[firstTouchI],invalid=dir==='LONG'?c.low<=sweep.extreme:c.high>=sweep.extreme;
    if(invalid)continue;
