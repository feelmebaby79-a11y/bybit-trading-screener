...
function classify(x){const vals=[x.oi5,x.oi15,x.oi1h,x.oi4h],persistent=vals.filter(Number.isFinite).length>=2&&vals.filter(Number.isFinite).every(v=>v>0),acceleration=Number.isFinite(x.oi1)&&Number.isFinite(x.oi5)?x.oi1*5>x.oi5*1.2:Number.isFinite(x.oi15)&&Number.isFinite(x.oi1h)&&x.oi15>0&&x.oi1h>0&&x.oi15*4>x.oi1h*1.2;const surge=Number.isFinite(x.oi1)&&x.oi1>=1;return{hit:surge,level:surge?"SURGE":null,persistent,acceleration}}
