const BYBIT_BASE="https://api.bybit.com",STATE_TTL_SECONDS=172800,SNAPSHOT_TTL_SECONDS=21600,OI_ALERT_THRESHOLD_1M=30,MIN_VOLUME_1M_TURNOVER_USD=100000;
...
export async function runScheduledOiSurgeWatch(env,scheduledTime=Date.now()){
 ...
 for(const symbol of Object.keys(cur))try{
  ...
  const c=classify(x);if(!c.hit)continue;Object.assign(x,c);Object.assign(x,await fetch1mVolumeContext(symbol));
  if(!Number.isFinite(x.volume1m_turnover)||x.volume1m_turnover<MIN_VOLUME_1M_TURNOVER_USD)continue;
  hits.push(x);
 ...
 return{ok:true,generated_at:new Date().toISOString(),universe_source:"BYBIT_LINEAR_ALL_USDT",alert_threshold_1m_pct:OI_ALERT_THRESHOLD_1M,min_volume_1m_turnover_usd:MIN_VOLUME_1M_TURNOVER_USD,checked_symbols:Object.keys(cur).length,hits,telegram_sent:sent,errors};
}
