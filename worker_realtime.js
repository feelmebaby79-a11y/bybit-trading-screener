...
async function runSyntheticEntryTest(env,symbol){const last=await fetchTicker(symbol),risk=Math.max(Math.abs(last)*.002,1e-12),item={symbol,direction:"LONG",score:100,poi_tf:"1H/15m"},t={tf:"5m",zone:{kind:"FVG"},entry_zone:[last-risk*.25,last+risk*.25],entry_mid:last,structural_sl:last-risk,tp1:last+2*risk,tp2:last+3*risk,rr_tp1:2,rr_tp2:3,nearest_liquidity:last+2.5*risk,expansion_room_r:2.5,event_id:`TEST:${symbol}:${Date.now()}`};await sendTelegram(env,buildEntryMessage(item,t,true));return{ok:true,test:true,type:"entry",symbol,telegram_sent:true,event_id:t.event_id};}
