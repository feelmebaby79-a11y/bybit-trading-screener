...
def score_100(row,direction):
    """Exact user-facing 100pt rubric: HTF25 + 1H15 + 15m20 + 5m15 + location10 + RR10 + market5."""
    if not row or direction not in {"LONG","SHORT"}:return None
    s=direction.lower(); t=trend(direction); total=0.0
    # HTF alignment 25: 1D 13 + 4H 12
    total += 13 if row.get("1D")==t else 0
    total += 12 if row.get("4H")==t else 0
    # 1H structure/location 15: structure 8 + favorable dealing range 7 (equilibrium gets 3)
    ...
    # 15m ICT 20: directional structure + sweep + MSS + displacement
    total += 5 if row.get("15m")==t else 0
    total += 5 if truthy(row.get(f"{s}_sweep15")) else 0
    total += 5 if truthy(row.get(f"{s}_mss15")) else 0
    total += 5 if truthy(row.get(f"{s}_disp15")) else 0
    # 5m FVG/OB + retracement 15. Scanner exposes directional FVG, trigger strength and fully-ready state.
    fvg=str(row.get(f"{s}_fvg5") or "").lower(); trig=num(row.get(f"{s}_5m_trigger")) or 0
    total += 7 if fvg==t else 0
    total += min(max(trig,0),4)
    total += 4 if truthy(row.get(f"{s}_entry_model_ready")) else 0
    # Entry location/chase risk 10
    chase=max(num(row.get(f"{s}_chase_penalty")) or 0,0)
    total += max(0,10-min(chase,5)*2)
    # R:R 10
    rr=num(row.get(f"{s}_rr"))
    if rr is not None:
        total += 10 if rr>=3 else 8 if rr>=2 else 6 if rr>=1.5 else 3 if rr>=1 else 0
    # BTC/market environment 5: relative strength persistence vs BTC in intended direction
    ...
    total += 5 if wins>=2 else 2 if wins==1 else 0
    return int(round(max(0,min(100,total))))

def choose_side(row):
    if not row:return None,None
    scored=[(score_100(row,d),d) for d in ("LONG","SHORT")]
...
def add(sym,d,bucket,extra=None):
    ...
    note="Current Bybit open position; always included while open." if bucket=="CURRENT_POSITION" else "POLYX-style early momentum candidate; strict 5m trigger still required." if bucket=="EARLY_MOMENTUM" else "Daily recommendation selected by exact 100-point rubric." if bucket=="DAILY_RECOMMENDATION" else "Core default symbol; monitored continuously."
...
for sym in position_symbols:
    d=position_side.get(sym,"LONG")
    existing=next((x for x in items if x["symbol"]==sym and x["direction"]==d),None)
...
for sym,row in by_symbol.items():
    ...
    # Do not force count: require real watch value, HTF alignment and minimum live/scanner RR.
...
payload={"ok":True,"version":15,"updated_at":now,"policy":{"mode":"CORE_PLUS_CURRENT_POSITIONS_PLUS_DAILY_RECOMMENDATIONS_PLUS_EARLY_MOMENTUM","default_symbols":DEFAULT_SYMBOLS,"include_current_positions":True,"position_source":position_source,"position_fetch_ok":position_fetch_ok,"position_symbols":position_symbols,"daily_recommendation_count":len([x for x in items if x["bucket"]=="DAILY_RECOMMENDATION"]),"daily_recommendation_max":MAX_DAILY_RECOMMENDATIONS,"minimum_watch_score":MIN_WATCH_SCORE,"early_momentum_count":len([x for x in items if x.get("early_momentum")]),"score_scale":100,"score_formula":{"HTF_alignment":25,"1H_structure_location":15,"15m_ICT":20,"5m_FVG_OB_retracement":15,"entry_location_chase_risk":10,"RR":10,"BTC_market_environment":5},"no_retrace_no_trade":True,"poi_arrival_is_entry":False,"poi_timeframes":["1H","15m"],"entry_timeframe":"5m","entry_model":"1H/15m POI -> touch -> confirmed 5m liquidity sweep -> MSS/CHoCH + displacement -> strict FVG/validated OB first retracement -> live RR >= 1.5","minimum_rr":1.5},"items":items}
OUTPUT.write_text(json.dumps(payload,ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
print(json.dumps({"ok":True,"version":15,"positions":position_symbols,"daily":[(x["symbol"],x["direction"],x["score"]) for x in items if x["bucket"]=="DAILY_RECOMMENDATION"],"early":[(x["symbol"],x["direction"],x["score"]) for x in items if x.get("early_momentum")]},ensure_ascii=False))