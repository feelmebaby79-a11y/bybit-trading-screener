...
def score_100(row, direction, raw_score):
    if raw_score is None or not row:
        return None
    ...
    rr_bonus = 5 if rr is not None and rr >= 3 else 4 if rr is not None and rr >= 2 else 3 if rr is not None and rr >= 1.5 else 1 if rr is not None and rr >= 1 else 0
...
def make_item(symbol, direction, raw_score, row, bucket, now):
    if bucket == "DEFAULT":
        note = "Core default symbol; always monitored. Direction and 100-point score are refreshed from the daily scan."
        ...
        note = "Daily recommendation; selected and ranked by the 100-point score and replaced by the next daily scan."
    return {
        "symbol": symbol,
        "direction": direction,
        "score": score_100(row, direction, raw_score),
        "score_scale": 100,
...
position_records = extract_position_records(scan) if position_fetch_ok else []

# Primary source: explicit side on each position record.
...
# Important fallback: the scanner always persists authoritative open-position symbols
# even when the detailed position record only contains analysis/market data and omits side.
# Never silently drop those positions from the watchlist. Infer monitoring direction from
# the fresh scanner row only for the missing-side case, while marking the source explicitly.
...
for symbol in scan_position_symbols:
    if symbol in position_side_by_symbol:
        continue
    direction, _ = choose_side(by_symbol.get(symbol))
    if direction:
        position_side_by_symbol[symbol] = direction
...
for symbol in DEFAULT_SYMBOLS:
    ...
    if direction:
        add_item(symbol, direction, raw, row, "DEFAULT")
    else:
...
for symbol in position_symbols:
    row = by_symbol.get(symbol)
    direction = position_side_by_symbol.get(symbol)
    if not direction:
        # If a position symbol exists but no side can be resolved, do not silently omit it.
        # Add both monitoring directions with no score rather than losing the open position.
        add_item(symbol, "LONG", None, row, "CURRENT_POSITION")
        add_item(symbol, "SHORT", None, row, "CURRENT_POSITION")
        continue
    scan_dir, scan_raw = choose_side(row)
    raw = scan_raw if scan_dir == direction else None
    matched = [i for i in items if i["symbol"] == symbol and i["direction"] == direction]
    if matched:
        matched[0]["bucket"] = "CURRENT_POSITION"
        matched[0]["note"] = "Current Bybit open position; always included while the position remains open, regardless of scanner rank."
    else:
        add_item(symbol, direction, raw, row, "CURRENT_POSITION")
...
for symbol, row in by_symbol.items():
    ...
    if display is not None:
        ranked.append((display, raw, symbol, direction, row))
ranked.sort(key=lambda x: (x[0], x[1]), reverse=True)
for display, raw, symbol, direction, row in ranked[:DAILY_RECOMMENDATION_COUNT]:
    add_item(symbol, direction, raw, row, "DAILY_RECOMMENDATION")
...
payload = {
    ...
    "policy": {
        ...
        "position_side_fallback": "scanner_direction_when_position_record_side_missing",
...
with OUTPUT.open("w", encoding="utf-8") as f:
    ...
    f.write("\n")
print(json.dumps({"ok": True, "position_fetch_ok": position_fetch_ok, "position_symbols": position_symbols, "item_count": len(items)}, ensure_ascii=False))
